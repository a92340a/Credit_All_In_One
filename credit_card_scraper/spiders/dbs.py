...
class DbsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield scrapy.Request(url, callback=self.parse_dbs, dont_filter=True)


    def parse_dbs(self,response):
        boxes = response.css('article.block.rounded.bg-white.shadow')
        for box in boxes:
            source = '星展'
            bank_name = '星展, 星展銀行, 810, dbs'
            ...
            if not card_content:
                ...
                card_content = ''.join(content).replace('回饋','回饋,').replace('上限','上限,')
            card_link = 'https://www.dbs.com.tw/personal-zh/cards/dbs-credit-cards/default.page'
            create_dt = today
