...
class AmericanSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        yield scrapy.Request(url, callback=self.parse_american, dont_filter=True)


    def parse_american(self,response):
        boxes = response.css('article.block.rounded.bg-white.shadow')
        for box in boxes:
            source = '美國運通'
            bank_name = '美國運通, american express'
            ...
            if not card_content:
                ...
                card_content = ''.join(content).replace('回饋','回饋,').replace('上限','上限,')
            card_link = 'https://www.americanexpress.com/zh-tw/credit-cards/all-cards/'
            create_dt = today
