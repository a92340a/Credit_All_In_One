...
class CtbcSpider(scrapy.Spider):
    name = "ctbc"
    allowed_domains = ["https://www.money101.com.tw"]
    ...
    def parse_ctbc(self,response):
        ...
        for box in boxes:
            source = '中信'
            bank_name = '中信, 中國信託銀行, 肥牡蠣, 牡蠣, 822, ctbc, we are family'
            ...
            if not card_content:
                ...
                card_content = ''.join(content).replace('回饋','回饋,').replace('上限','上限,')
            card_link = 'https://www.ctbcbank.com/twrbo/zh_tw/cc_index/cc_product/cc_introduction_index.html'
            create_dt = today
